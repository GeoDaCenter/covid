...
def parseCsv(fileInfo):
    ...
    if fileInfo['scale'] == 0:
        ...
        scaleSuffix = f'.e-{fileInfo["scale"]}'
    ...
    for i in range(0, len(csvData)):
        ...
        for val in list(csvData.iloc[i].values)[fileInfo['dateIndex']:]:
            if np.isnan(val) or val == nullVal:
                cleanVals.append(-999)
            else:
                cleanVals.append(int(round(val*(10**fileInfo['scale']))))

        rowObj[i].vals.extend(cleanVals)

    f = open(os.path.join(repo_root, f'public/pbf/{fileInfo["fileName"]}{scaleSuffix}.pbf'), "wb")
