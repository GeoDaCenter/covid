...
> Maintainer note, Jan 2025: This repository has been archived, and is no longer updated. See [geodacenter/covid-v3](https://github.com/geodacenter/covid-v3) for the production US Covid Atlas site, published through Netlify. However, to facilitate redirects from formerly published urls (geodacenter.github.io/covid), Github Pages is still enabled via Workflow on this repository. This is only so that the CNAME record here can signify a permanent redirect (301) to the correct domain: [uscovidatlas.org](https://uscovidatlas.org). Note that CNAME doesn't *look* properly configured ("DNS check unsuccessful"), but it still seems to issue the redirect. If this stops working, then the CNAME record can be removed and the Github Pages build will fallback to the JS redirects in `*.html` pages.
