...
export const generateReport = (geoids, state) => {
    ...
    try {
        if (state.storedData.hasOwnProperty(currentTables.testing?.file)){
            report.wk_pos = Math.round(aggregateTimeseries(state.storedData[currentTables.testing_wk_pos.file].data, properties, geoids, state.dataParams.nIndex, 'weighted_average')*10000)/100
            ...
            report.testing = Math.round(aggregateTimeseries(state.storedData[currentTables.testing.file].data, properties, geoids, state.dataParams.nIndex, 'sum')*100)/100
            report.ccpt = Math.round(aggregateTimeseries(state.storedData[currentTables.testing_ccpt.file].data, properties, geoids, state.dataParams.nIndex, 'weighted_average')*10000)/100
        }    
    } catch {}
