...
const VariablePanel = (props) => {
  ...
  if (props.tables && props.columns) {
    props.tables.forEach((table) => {
      ...
      menuItems.push(
        <ListSubheader value={table} key={table}>
          {PlainEnglishNames[table]}
        </ListSubheader>,
      );
      if (props.columns[table]) {
        if (Date.parse(props.columns[table].slice(-1)[0])) {
          menuItems.push(
            <MenuItem value={`${table}-${table}-cols`} key={`${table}-cols`}>
              {table}
            </MenuItem>,
          );
        } else {
          props.columns[table].forEach((column) => {
            if (!ignoreColumns.includes(column))
              menuItems.push(
                <MenuItem
                  value={`${table}-${column}-characteristic`}
                  key={column}
                >
                  {column}
                </MenuItem>,
        ...
        menuItems.push(
          <MenuItem
            value={`${table}-population-characteristic`}
            key={'population'}
          >
            Population
          </MenuItem>,
        ...
        menuItems.push(
          <MenuItem value={`${table}-beds-characteristic`} key={'beds'}>
            Beds
          </MenuItem>,
        );
      }
  ...
  return (
    <div
      id="variable-panel"
      style={{ transform: hidePanel ? 'translateX(-100%)' : '' }}
    >
      <p>Variable</p>
