...
const Tab = styled.button`
    ...
    font-family:${props => props.big ? 'Playfair Display, script' : 'Lato, sans-serif'};
...
const RenderJenksEx = ({fills=[]}) => <JenksEx>
    <div>
       ...
       <MethodsPage>
           ...
           <ContentContainer>
                ...
                <TabBar position="static">
                    <Tab big={true} {...a11yProps(0)} active={tabValue === 0} onClick={() => setTabValue(0, 'methodology')}>Mapping</Tab>
                    <Tab big={true} {...a11yProps(1)} active={tabValue === 1} onClick={() => setTabValue(1, 'methodology')}>Infrastructure</Tab>
                    <Tab big={true} {...a11yProps(2)} active={tabValue === 2} onClick={() => setTabValue(2, 'methodology')}>Data Collection</Tab>
                </TabBar>
                <TabPanel display={tabValue===0}>
                    <h2>Choropleth Mapping: Natural Breaks</h2>
                    <p>
                        For geo-visualization, we adopt jenks natural breaks maps to classify the values of the pandemic data for each state/county. 
                        A natural breaks map uses a nonlinear algorithm to create groups where within-group homogeneity is maximized, grouping and 
                        highlighting extreme observations.
                        <br/><br/>
                        The Jenks break is a method designed by Jenks (1957) which assigns each observation to the best classification it could possibly
                        belong to. It aims to find the best possible arrangement of the geographical units into classes, such that the deviation from the 
                        mean within classes is minimize, while the differences between classes in maximized. In other words, it aims to group more alike 
                        geographical units, while trying to maximize the differences between the groups created.
                        <br/><br/>
                        Here's the Jenks strategy in action: 
                    </p>
                    <Gutter h={20}/>
                    <TabBar position="static">
                        <Tab {...a11yProps(0)} active={jenksTab === 0} onClick={() => setJenksTab(0, 'natural-jenks-explainer')}>Goal</Tab>
                        <Tab {...a11yProps(1)} active={jenksTab === 1} onClick={() => setJenksTab(1, 'natural-jenks-explainer')}>Step 1</Tab>
                        <Tab {...a11yProps(2)} active={jenksTab === 2} onClick={() => setJenksTab(2, 'natural-jenks-explainer')}>Step 2</Tab>
                        <Tab {...a11yProps(3)} active={jenksTab === 3} onClick={() => setJenksTab(3, 'natural-jenks-explainer')}>Step 3</Tab>
                        <Tab {...a11yProps(4)} active={jenksTab === 4} onClick={() => setJenksTab(4, 'natural-jenks-explainer')}>Step 4</Tab>
                    </TabBar>
                    <TabPanel display={jenksTab===0}>
                        <div className="container">
                            <h4>For illustration purposes only, assume that we want to classify three counties based on its number of covid19 cases into two groups:</h4>
                            <RenderJenksEx/>
                            <p>Suppose DuPage and Will Counties have 1 case, and Cook County has 4 cases.</p>
                        </div>
                    </TabPanel>  
                    <TabPanel display={jenksTab===1}>
                        <div className="container">
                            <h4>To start, calculate the average of all counties and the sum of square deviations from that average:</h4>
                            <RenderJenksEx/>
                            <code>Average: 6 / 3 = <b>2</b></code>
                            <br/><br/>
                            <code>Sum of squares: (4<sup>2</sup>+1<sup>2</sup>+1<sup>2</sup>) - ((4+1+1)<sup>2</sup> / 3) = <b>6</b></code>
                        </div>
                    </TabPanel>                          
                    <TabPanel display={jenksTab===2}>
                        <div className="container">
                            <h4>Next, try all possible arrangements of counties into groupings, and then calculate the sum of squares for each possible grouping.</h4>
                            <br/><br/>
                            <h4>Option 1</h4>
                            ...
                            <br/><br/>
                            <h4>Option 2</h4>
                            <RenderJenksEx fills={[colors.lightblue, colors.yellow, colors.lightblue]}/>
                            <code>Sum of squares:</code>
                                <br/>
                    ...
                    <TabPanel display={jenksTab===3}>                    
                        <div className="container">
                            <h4>Calculate the difference between each option's sum of squares and the total overall sum of squares. The higher this value is, 
                                the greater the difference between groups. We'll choose the option that best maximizes difference.
                            </h4><br/><br/>
                            <h4>Option 1</h4>
                            <RenderJenksEx fills={[colors.yellow, colors.yellow, colors.lightblue]}/>
                    ...
                    <TabPanel display={jenksTab===4}>
                        <div className="container">
                            <h4>
                                In this case, option 1 maximizes difference, and these are the selected bins:
                            </h4><br/><br/>
                            <RenderJenksEx fills={[colors.yellow, colors.yellow, colors.lightblue]}/>
                            ...
                            <br/>
                            <p>
                                A formal description of the Jenks break could be found here: Jenks, GF (1967). 
                                The Data Model Concept in Statistical Mapping. International Yearbook of Cartography. 7: 186–190.
                            </p>
                        </div>
                    </TabPanel>      
                    <Gutter h={40}/>
                    ...
                    <br/>
                    <p>
                        The Local Clustering maps identify clusters or collections of geographical units similar, in statistical terms, based on the indicator used. They can be used to identify hot spots of cold spots across space. Hot spots are of particular 
                        interests in epidemiological analysis such as the spread of covid19, as it allows the identification of “hot” groups of areas (e.g. States, counties) significantly affected by the virus. It is, collection of counties, for instance, 
                        with a relatively high indicator which are also surrounded by areas of high indicators. In the maps, such areas are represented by the red colors.
                    </p>
                        
                    <Gutter h={40} />
                    <h2>Interactive Local Moran's I Tutorial</h2>
                    <p>
                        Use the interactive below to learn more about how spatial contiguity weights and LISA statistics are calculated:
                    </p>
                    <iframe frameborder="0"
    src="https://observablehq.com/embed/@michelleeesi/local-spatial-autocorrelation-interactive-tutorial?cell=*"></iframe>
                    <br/>
                    <ul>
                        <li>
                            For more details:
