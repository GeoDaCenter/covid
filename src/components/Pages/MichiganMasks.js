...
export default function MichiganMasks(){
    ...
    const getData = async() => {
        ...
        for (let i=0; i<geo.features.length; i++){
          if (geo.features[i].properties.GEOID >= 26000 && geo.features[i].properties.GEOID < 27000){
            let weeklyAverage = data.find(f => f.fips === geo.features[i].properties.GEOID)?.weeklyAverage / 7
            weeklyAverages.push(( weeklyAverage / geo.features[i].properties.population ) * 1e5)
            filteredFeatures.push({
               ...geo.features[i],
               properties: {
                  ...geo.features[i].properties,
                  averagePer100k: ( weeklyAverage / geo.features[i].properties.population ) * 1e5,
                  weeklyAverage,
               }
            })
          }
    ...
    const michiganLayers = [
        ...
        new ScatterplotLayer({
            ...
            getRadius: d => activeSchool.Name === d.Name ? 20 : 5,
    ...
    const fitMichigan = {
        ...fitBounds({
            ...
            bounds: [[-89.274902,41.145570],[-81.485596,46.758680]]
          }),
        bearing:0,
        pitch:0
    }
       ...
       <MichiganPage >
            ...
            <Grid container spacing={3}>
                <Grid item xs={12} md={9}>
                    <DeckContainer>
                        {(bins.length && maskData.length && geoData.features?.length) ? <DeckGL
                    ...
                    <Legend>
                        <LegendLi>
                            <svg viewBox="0 0 10 10" xmlns="http://www.w3.org/2000/svg">
                                <rect width="10" height="10" fill={`rgb(${colors[0].join(',')})`}/>
                            </svg>
                            {'<'}
                            {Math.round(bins[0]*10)/10}
                        </LegendLi>
                        {bins.map((bin, i) => 
                            i < bins.length-1 ? <LegendLi>
                                <svg viewBox="0 0 10 10" xmlns="http://www.w3.org/2000/svg">
                                    <rect width="10" height="10" fill={`rgb(${colors[i+1].join(',')})`}/>
                                </svg>
                                {Math.round(bins[i]*10)/10}-{Math.round(bins[i+1]*10)/10}
                                </LegendLi> : null
                        )}
                        <LegendLi>
                            <svg viewBox="0 0 10 10" xmlns="http://www.w3.org/2000/svg">
                                <rect width="10" height="10" fill={`rgb(${colors[colors.length-1].join(',')})`}/>
                            </svg>
                            {'>'}
