...
export default function MapTooltipContent() {
  ...
  if (tooltipContent.data.custom) {
    return (
      ...
      >
        {Object.entries(tooltipContent.data.custom).map((entry) => (
          <>
            <b>{entry[0]}:</b>{' '}
    ...
    >
      ...
      {tooltipContent.data.vaccines_fully_vaccinated !== undefined && (
        <>
          Fully Vaccinated:{' '}
          {Math.round(
            (tooltipContent.data.vaccines_fully_vaccinated /
              tooltipContent.data.population) *
              1000,
          ) / 10}
          %<br />
        </>
      ...
      {tooltipContent.data.vaccines_one_dose !== undefined && (
        <>
          At least one dose:{' '}
          {Math.round(
            (tooltipContent.data.vaccines_one_dose /
              tooltipContent.data.population) *
              1000,
          ) / 10}
          %<br />
      ...
      {tooltipContent.data.hasOwnProperty('cases') && (
        <>
          Cases: {tooltipContent.data.cases?.toLocaleString('en') || 0}
          <br />
        </>
      )}
      {tooltipContent.data.hasOwnProperty('deaths') && (
        <>
          Deaths: {tooltipContent.data.deaths?.toLocaleString('en') || 0}
          <br />
        </>
      )}
      {tooltipContent.data.hasOwnProperty('daily_cases') && (
        <>
          Daily New Cases:{' '}
          {tooltipContent.data.daily_cases?.toLocaleString('en') || 0}
          <br />
        </>
      )}
      {tooltipContent.data.hasOwnProperty('daily_deaths') && (
        <>
          Daily New Deaths:{' '}
          {tooltipContent.data.daily_deaths?.toLocaleString('en') || 0}
          <br />
        </>
      )}
      <br />
      {tooltipContent.data.hasOwnProperty('testing_wk_pos') && (
        <>
          7-Day Average Positivity Rate:{' '}
          {Math.round(tooltipContent.data?.testing_wk_pos * 10000) / 100 || 0}%
          <br />
        </>
      )}
      {tooltipContent.data.hasOwnProperty('testing_tcap') && (
        <>
          7-Day Average Tests Performed:{' '}
          {tooltipContent.data.testing_tcap?.toLocaleString('en') || 0} per 100k
          <br />
        </>
      )}
      ...
      {tooltipContent.data['Hospital Type'] && (
        <>
          ...
          {tooltipContent.data.Address_2 &&
            `${tooltipContent.data.Address_2}${(<br />)}`}
          {tooltipContent.data.City}, {tooltipContent.data.State}
          <br />
          {tooltipContent.data.Zipcode}
          <br />
        </>
      )}
      ...
      {'volume' in tooltipContent.data && (
        <>
          <h3>{tooltipContent.data.name}</h3>
          {tooltipContent.data.type === 0 && (
            <>
              <b>Invited</b> vaccination clinic
            </>
          )}
          {tooltipContent.data.type === 1 && (
          ...
          {tooltipContent.data.volumne && (
            <>
              <br />
              <br />
              Expected Vaccination Volume: {tooltipContent.data.volume}/day
              <br />
              <br />
            </>
          )}
