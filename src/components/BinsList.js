...
const BinsList = (props) => {
    const bins = "string" === typeof props.data[0] ? props.data : props.data.map(bin => `${Math.round(bin*10)/10}`)
    // Return div with bins, tooltip if using
    return (
        (bins.map((bin, index) => 
            <div className={`bin label ${bins.some(bin => bin.includes('tooltip')) ? 'tooltipText' : ''}`} key={`${bin}_${index}`}>
