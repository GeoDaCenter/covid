...
export default function MapTooltipContent(){
    ...
    return <HoverDiv style={{position: 'absolute', zIndex: 1, pointerEvents: 'none', left: tooltipContent.x, top: tooltipContent.y}}>
        ...
        {cases !== undefined && <>Cases: {cases.toLocaleString('en')}<br/></>}
        {deaths !== undefined && <>Deaths: {deaths.toLocaleString('en')}<br/></>}
        {daily_cases !== undefined && <>Daily New Cases: {daily_cases.toLocaleString('en')}<br/></>}
        {daily_deaths !== undefined && <>Daily New Deaths: {daily_deaths.toLocaleString('en')}<br/></>}
        <br/>
        {testing_wk_pos !== undefined && <>7-Day Positivity Rate: {testing_wk_pos.toLocaleString('en')}<br/></>}
        {testing_tcap !== undefined && <>7-Day Testing Capacity: {testing_tcap.toLocaleString('en')}<br/></>}
