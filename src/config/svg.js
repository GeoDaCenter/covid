export const compact = (
  <svg x="0px" y="0px" viewBox="0 0 19.6 32">
    <path d="M1,0h17.6c0.6,0,1,0.5,1,1v3.1c0,0.6-0.4,1-1,1H1c-0.6,0-1-0.5-1-1V1C0,0.5,0.4,0,1,0z" />
    <path d="M1,9h17.6c0.6,0,1,0.5,1,1v3.1c0,0.6-0.4,1-1,1H1c-0.6,0-1-0.5-1-1V10C0,9.4,0.4,9,1,9z" />
    <path d="M1,17.9h17.6c0.6,0,1,0.5,1,1V22c0,0.6-0.4,1-1,1H1c-0.6,0-1-0.5-1-1v-3.1C0,18.4,0.4,17.9,1,17.9z" />
    <path d="M1,26.9h17.6c0.6,0,1,0.5,1,1V31c0,0.6-0.4,1-1,1H1c-0.6,0-1-0.5-1-1v-3.1C0,27.3,0.4,26.9,1,26.9z" />
  </svg>
);

export const expand = (
  <svg x="0px" y="0px" viewBox="0 0 19.6 32">
    <path d="M1,0h17.6c0.6,0,1,0.5,1,1v3.1c0,0.6-0.4,1-1,1H1c-0.6,0-1-0.5-1-1V1C0,0.5,0.4,0,1,0z" />
    <path d="M1,9h17.6c0.6,0,1,0.5,1,1v21c0,0.6-0.4,1-1,1H1c-0.6,0-1-0.5-1-1V10C0,9.4,0.4,9,1,9z" />
  </svg>
);

export const settings = (
  <svg viewBox="0 0 50 50">
    <circle
      className="cls-1"
      cx="25.43"
      cy="24.63"
      r="15.4"
      transform="translate(-3.28 3.92) rotate(-8.28)"
    />
    <path
      className="cls-2"
      d="M28.87,47.49l-7.07,0L22,41.58a17.18,17.18,0,0,0,6.74,0Z"
    />
    <path
      className="cls-2"
      d="M28.84,7.68a17.18,17.18,0,0,0-6.74,0l-.16-5.89H29Z"
    />
    <path
      className="cls-2"
      d="M44,38.35l-5,5L35,39a17.33,17.33,0,0,0,4.78-4.76Z"
    />
    <path
      className="cls-2"
      d="M15.85,10.22A17.19,17.19,0,0,0,11.07,15L6.8,10.92l5-5Z"
    />
    <path
      className="cls-2"
      d="M48.27,21.17l0,7.08-5.88-.19a17.23,17.23,0,0,0,0-6.75Z"
    />
    <path
      className="cls-2"
      d="M8.45,21.2a17.37,17.37,0,0,0,0,6.74l-5.89.16,0-7.08Z"
    />
    <path
      className="cls-2"
      d="M39.12,6l5,5-4.29,4a17.43,17.43,0,0,0-4.75-4.78Z"
    />
    <path
      className="cls-2"
      d="M11,34.2A17.16,17.16,0,0,0,15.75,39L11.7,43.25l-5-5Z"
    />
  </svg>
);

export const report = (
  <svg viewBox="0 0 50 50">
    <rect x="0.03" y="0.19" width="49.93" height="3.85" rx="1" />
    <rect x="0.03" y="8.4" width="46.19" height="3.85" rx="1" />
    <rect x="0.03" y="21.12" width="49.93" height="3.85" rx="1" />
    <rect x="0.03" y="29.58" width="49.93" height="3.85" rx="1" />
    <rect x="0.03" y="38.04" width="43.69" height="3.85" rx="1" />
    <rect x="0.03" y="46.15" width="49.93" height="3.85" rx="1" />
  </svg>
);

export const chart = (
  <svg x="0px" y="0px" viewBox="0 0 100 100">
    <g>
      <path
        d="M52.5,21.4c-1.9,0-3.6,1.3-4.1,3.1L37.9,63.7l-6.4-11.1c-1.2-2-3.7-2.7-5.7-1.5c-0.3,0.2-0.6,0.4-0.9,0.7
                L10.1,66.6c-1.7,1.6-1.7,4.2-0.2,5.9c1.6,1.7,4.2,1.7,5.9,0.2c0.1,0,0.1-0.1,0.1-0.1L27,61.5l8.7,15.1c1.2,2,3.7,2.7,5.7,1.5
                c0.9-0.6,1.6-1.5,1.9-2.5l9.1-33.9l4.6,17.2c0.6,2.2,2.9,3.5,5.1,2.9c1.1-0.3,2-1,2.5-1.9l10.4-18l8.9,9.4c1.6,1.7,4.2,1.8,5.9,0.3
                s1.8-4.2,0.3-5.9c0,0-0.1-0.1-0.1-0.1L77.3,32.1c-1.6-1.7-4.2-1.8-5.9-0.2c-0.3,0.3-0.6,0.6-0.8,1L62.5,47l-6-22.5
                C56,22.7,54.4,21.4,52.5,21.4L52.5,21.4z"
      />
    </g>
  </svg>
);

export const info = (
  <svg viewBox="0 0 100 100" x="0px" y="0px">
    <g>
      <path d="M 62.0763 27.4552 C 64.0258 25.642 65 23.4406 65 20.8589 C 65 18.2815 64.0258 16.0809 62.0763 14.2511 C 60.1273 12.4207 57.7859 11.5 55.0413 11.5 C 52.3076 11.5 49.9438 12.4207 47.9833 14.2511 C 46.0343 16.0809 45.0487 18.2815 45.0487 20.8589 C 45.0487 23.4406 46.0343 25.642 47.9833 27.4552 C 49.9438 29.2682 52.3076 30.178 55.0413 30.178 C 57.7859 30.178 60.1273 29.2682 62.0763 27.4552 ZM 57.5841 88.0802 C 61.1017 86.4348 62.9616 83.3419 61.1353 81.9274 C 60.0823 81.1132 58.7041 82.4604 57.6963 82.4604 C 55.5343 82.4604 54.0103 82.1065 53.1367 81.3939 C 52.2518 80.6754 51.8261 79.3446 51.8261 77.3796 C 51.8261 76.5942 51.9493 75.4433 52.2182 73.9213 C 52.487 72.395 52.8007 71.0302 53.1367 69.8404 L 57.3153 55.0418 C 57.7073 53.683 57.9988 52.1893 58.1554 50.5672 C 58.301 48.9276 58.3798 47.7935 58.3798 47.1533 C 58.3798 44.0378 57.2817 41.5004 55.0971 39.5465 C 52.9237 37.5991 49.8094 36.6159 45.7765 36.6159 C 43.5361 36.6159 41.1501 36.9472 38.652 37.8117 C 33.7564 39.5293 34.8432 43.7968 35.9296 43.7968 C 38.1364 43.7968 39.6152 44.1722 40.3995 44.9193 C 41.1837 45.6604 41.5868 46.9796 41.5868 48.8828 C 41.5868 49.9269 41.4413 51.1007 41.1947 52.3689 C 40.9369 53.6381 40.635 54.9909 40.2541 56.4111 L 36.053 71.2659 C 35.6947 72.8267 35.4253 74.2246 35.2463 75.4648 C 35.0784 76.7058 35 77.9187 35 79.1091 C 35 82.1578 36.12 84.6722 38.3716 86.6596 C 40.6238 88.6528 44.0854 90.5 48.1405 90.5 C 50.7731 90.5 54.537 89.518 57.5841 88.0802 Z"></path>
    </g>
  </svg>
);

export const verticalGrip = (
  <svg viewBox="0 0 12.1 50">
    <rect width="2.4" height="50" />
    <rect x="4.9" width="2.4" height="50" />
    <rect x="9.8" width="2.4" height="50" />
  </svg>
);

export const selectRect = (
  <svg viewBox="0 0 100 100">
    <g transform="translate(50 50) scale(0.69 0.69) rotate(0) translate(-50 -50)">
      <svg fill="#000000" viewBox="0 0 8.4666665 8.4666669">
        <g transform="translate(0,-288.53333)">
          <path d="m 0.52916618,289.0625 v 1.05833 H 1.0583328 v -0.52916 h 0.5291667 v -0.52917 z m 1.58750012,0 v 0.52917 h 1.0583333 v -0.52917 z m 1.5875,0 v 0.52917 h 1.0583333 v -0.52917 z m 1.5875,0 v 0.52917 h 1.0583333 v -0.52917 z m 1.5875,0 v 0.52917 H 7.408333 v 0.52916 H 7.9374996 V 289.0625 Z M 0.52916618,290.65 v 1.05833 H 1.0583328 V 290.65 Z m 6.87916682,0 v 1.05833 H 7.9374996 V 290.65 Z m -6.87916682,1.5875 v 1.05833 H 1.0583328 v -1.05833 z m 6.87916682,0 v 1.05833 h 0.5291666 v -1.05833 z m -6.87916682,1.5875 v 1.05833 H 1.0583328 V 293.825 Z m 6.87916682,0 v 1.05833 H 7.9374996 V 293.825 Z m -6.87916682,1.5875 v 1.05833 H 1.5874995 v -0.52916 H 1.0583328 v -0.52917 z m 6.87916682,0 v 0.52917 H 6.8791663 v 0.52916 h 1.0583333 v -1.05833 z m -5.2916667,0.52917 v 0.52916 h 1.0583333 v -0.52916 z m 1.5875,0 v 0.52916 h 1.0583333 v -0.52916 z m 1.5875,0 v 0.52916 h 1.0583333 v -0.52916 z"></path>
        </g>
      </svg>
    </g>
  </svg>
);

export const locate = (
  <svg viewBox="0 0 100 100">
    <g>
      <path d="M50.2,34.3c-8.6,0-15.6,7-15.6,15.6c0,8.6,7,15.6,15.6,15.6c8.6,0,15.6-7,15.6-15.6   C65.8,41.3,58.8,34.3,50.2,34.3z M87.9,45.6C86,28.1,72,14.2,54.5,12.2v-8H47v7.9c-18,1.5-32.4,15.7-34.5,33.5H4.3v7.5h8.1   C14,71.5,28.6,86.2,47,87.7v8.1h7.5v-8.2c17.9-2,32-16.5,33.5-34.5h7.9v-7.5H87.9z M50.2,80.3c-16.8,0-30.4-13.7-30.4-30.4   c0-16.8,13.7-30.4,30.4-30.4c16.8,0,30.4,13.7,30.4,30.4C80.7,66.7,67,80.3,50.2,80.3z"></path>
    </g>
  </svg>
);

export const minus = (
  <svg viewBox="0 0 100 100">
    <rect x="20" y="43.1" width="60" height="13.6" />
  </svg>
);

export const plus = (
  <svg viewBox="0 0 100 100">
    <g>
      <polygon
        points="43.1,17.2 56.9,17.2 56.9,42.6 80,42.6 80,57.4 56.9,57.4 56.9,82.8 43.1,82.8 43.1,57.4 20,57.4 
                    20,42.6 43.1,42.6 	"
      />
    </g>
  </svg>
);

export const compass = (
  <svg viewBox="0 0 100 100">
    <path
      d="M49.8,90.3c-0.5,0-1.4-0.2-2-1.4l-0.1-0.2c0-0.1-0.1-0.1-0.1-0.2L34.2,53.2c-0.6-1.8-0.6-4.8,0-6.4l13.8-35.4
            c0.5-1.3,1.4-1.7,2.2-1.7c0.7,0,1.6,0.3,2.1,1.8l13.6,35.3c0.6,1.8,0.6,4.7,0,6.4L52,88.5C51.4,90.1,50.4,90.3,49.8,90.3
            C49.8,90.3,49.8,90.3,49.8,90.3z M49.8,88.1C49.8,88.1,49.8,88.1,49.8,88.1c0.1,0,0.1-0.1,0.1-0.2l13.8-35.4c0.5-1.3,0.5-3.8,0-5.2
            L50.1,12.1c0-0.1-0.1-0.2-0.1-0.2c0,0,0,0.1-0.1,0.2L36.2,47.4c-0.5,1.3-0.5,3.9,0,5.3L49.8,88.1z"
    />
    <path
      d="M64.7,47.1L51.1,11.8c-0.3-0.8-0.7-1.2-1.1-1.2c-0.4,0-0.9,0.3-1.1,1.1L35.2,47.1c-0.3,0.8-0.4,1.8-0.4,2.9h30.4
            C65.1,48.9,65,47.9,64.7,47.1z"
    />
  </svg>
);

export const share = (
  <svg x="0px" y="0px" viewBox="0 0 100 100">
    <path d="M22.5,65c4.043,0,7.706-1.607,10.403-4.208l29.722,14.861C62.551,76.259,62.5,76.873,62.5,77.5c0,8.284,6.716,15,15,15   s15-6.716,15-15c0-8.284-6.716-15-15-15c-4.043,0-7.706,1.608-10.403,4.209L37.375,51.847C37.449,51.241,37.5,50.627,37.5,50   c0-0.627-0.051-1.241-0.125-1.847l29.722-14.861c2.698,2.601,6.36,4.209,10.403,4.209c8.284,0,15-6.716,15-15   c0-8.284-6.716-15-15-15s-15,6.716-15,15c0,0.627,0.051,1.241,0.125,1.848L32.903,39.208C30.206,36.607,26.543,35,22.5,35   c-8.284,0-15,6.716-15,15C7.5,58.284,14.216,65,22.5,65z"></path>
  </svg>
);

export const print = (
  <svg viewBox="0 0 100 100">
    <path d="M86.00928,27.09229H78V8.16205A3.16207,3.16207,0,0,0,74.838,5H25.16211A3.16209,3.16209,0,0,0,22,8.16211V27.09229H14a9.02644,9.02644,0,0,0-9,9V69.86353a3.16917,3.16917,0,0,0,3.16919,3.16918H22V91.83789A3.16209,3.16209,0,0,0,25.16211,95H74.83789A3.16209,3.16209,0,0,0,78,91.83789V73.03271H91.822A3.178,3.178,0,0,0,95,69.85474V36.083A8.9907,8.9907,0,0,0,86.00928,27.09229ZM73.99951,91.00006h-48V58.05518h48Zm0-63.90777h-48V9.00006h48ZM81.33325,44.4256A3.66666,3.66666,0,1,1,85,40.759,3.66659,3.66659,0,0,1,81.33325,44.4256Z"></path>
    <rect x="33.56671" y="63.14117" width="32.86621" height="4.2002"></rect>
    <rect x="33.56671" y="72.42731" width="32.86621" height="4.2002"></rect>
    <rect x="33.56671" y="81.71393" width="32.86621" height="4.2002"></rect>
  </svg>
);

export const twitter = (
  <svg viewBox="0 0 248 204">
    <g>
      <path
        d="M221.95,51.29c0.15,2.17,0.15,4.34,0.15,6.53c0,66.73-50.8,143.69-143.69,143.69v-0.04
            C50.97,201.51,24.1,193.65,1,178.83c3.99,0.48,8,0.72,12.02,0.73c22.74,0.02,44.83-7.61,62.72-21.66
            c-21.61-0.41-40.56-14.5-47.18-35.07c7.57,1.46,15.37,1.16,22.8-0.87C27.8,117.2,10.85,96.5,10.85,72.46c0-0.22,0-0.43,0-0.64
            c7.02,3.91,14.88,6.08,22.92,6.32C11.58,63.31,4.74,33.79,18.14,10.71c25.64,31.55,63.47,50.73,104.08,52.76
            c-4.07-17.54,1.49-35.92,14.61-48.25c20.34-19.12,52.33-18.14,71.45,2.19c11.31-2.23,22.15-6.38,32.07-12.26
            c-3.77,11.69-11.66,21.62-22.2,27.93c10.01-1.18,19.79-3.86,29-7.95C240.37,35.29,231.83,44.14,221.95,51.29z"
      />
    </g>
  </svg>
);

export const facebook = (
  <svg viewBox="0 0 1024 1024">
    <path
      d="M1024,512C1024,229.2,794.8,0,512,0S0,229.2,0,512c0,255.6,187.2,467.4,432,505.8V660H302V512h130V399.2
        C432,270.9,508.4,200,625.4,200c56,0,114.6,10,114.6,10v126h-64.6c-63.6,0-83.4,39.5-83.4,80v96h142l-22.7,148H592v357.8
        C836.8,979.4,1024,767.6,1024,512z"
    />
  </svg>
);

export const hamburger = (
  <svg height="100px" width="100px" viewBox="0 0 32 32" x="0px" y="0px">
    <g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
      <g transform="translate(6.000000, 9.000000)" fill-rule="nonzero">
        <path d="M0,8 L0,6 L20,6 L20,8 L0,8 Z M0,14 L0,12 L20,12 L20,14 L0,14 Z M0,2 L0,0 L20,0 L20,2 L0,2 Z"></path>
      </g>
    </g>
  </svg>
);
export const close = (
  <svg height="100px" width="100px" viewBox="0 0 100 100" x="0px" y="0px">
    <g>
      <g>
        <polygon points="75 31 69 25 50 44 31 25 25 31 44 50 25 69 31 75 50 56 69 75 75 69 56 50"></polygon>
      </g>
      <g fill="#000000">
        <polygon points="75 31 69 25 50 44 31 25 25 31 44 50 25 69 31 75 50 56 69 75 75 69 56 50"></polygon>
      </g>
    </g>
  </svg>
);
export const embed = (
  <svg viewBox="0 0 512 512">
    <path d="M288 368l48 48 160-160-160-160-48 48 112 112z"></path>
    <path d="M224 144l-48-48-160 160 160 160 48-48-112-112z"></path>
  </svg>
);
export const alert = (
  <svg viewBox="0 0 24 24" x="0px" y="0px">
    <path d="M12,2A10,10,0,1,0,22,12,10,10,0,0,0,12,2Zm0,18a8,8,0,1,1,8-8A8,8,0,0,1,12,20Z"></path>
    <path d="M12,6a1,1,0,0,0-1,1v6a1,1,0,0,0,2,0V7A1,1,0,0,0,12,6Z"></path>
    <path d="M12.71,16.29a1,1,0,0,0-1.42,0,1,1,0,0,0-.21.33A1,1,0,0,0,11,17a1,1,0,0,0,.29.71,1,1,0,0,0,1.42,0A1,1,0,0,0,13,17a1,1,0,0,0-.08-.38A1,1,0,0,0,12.71,16.29Z"></path>
  </svg>
);
export const addData = (
  <svg viewBox="0 0 98.07 70.77">
    <path
      d="M89.26,41V32A17.5,17.5,0,0,0,71.75,14.48H18.38A17.5,17.5,0,0,0,.87,32V46.42A17.5,17.5,0,0,0,18.38,63.93H49.7A24.73,24.73,0,1,0,89.26,41Zm-15,38.12A18.55,18.55,0,1,1,92.76,60.53,18.6,18.6,0,0,1,74.22,79.08Z"
      transform="translate(-0.87 -14.48)"
    />
    <path
      class="cls-1"
      d="M85.14,57.44H77.31V49.61a3.09,3.09,0,0,0-6.18,0v7.83H63.3a3.09,3.09,0,0,0,0,6.18h7.83v7.83a3.09,3.09,0,1,0,6.18,0V63.62h7.83a3,3,0,0,0,3.09-3.09A3.1,3.1,0,0,0,85.14,57.44Z"
      transform="translate(-0.87 -14.48)"
    />
  </svg>
);
export const arrow = (
  <svg viewBox="0 0 100 100">
    <g>
      <path d="M31.3,97.5c-2.6,0-5.1-1-7.1-2.9c-3.9-3.9-3.9-10.3,0-14.2L54.5,50L24.2,19.6c-3.9-3.9-3.9-10.3,0-14.2    c3.9-3.9,10.3-3.9,14.2,0l37.5,37.5c1.9,1.9,2.9,4.4,2.9,7.1s-1.1,5.2-2.9,7.1L38.4,94.6C36.4,96.5,33.8,97.5,31.3,97.5z"></path>
    </g>
  </svg>
);
