...
const generateJoinData = ({
  ...
  if (!dataReady || !bins.breaks) return [
    undefined,
    undefined
  ]
  ...
  for (let i=0; i<geoids.length; i++) {
    joinData[geoids[i]] = {
      color: getContinuousColor(binData[i], bins.breaks, mapParams.colorScale, true),
      value: binData[i]
    }
  }
  return [
    joinData,
    maxDesirableHeight/Math.max(...binData)
  ]
}
...
export function useLisaMap({
    ...
    useMemo(async() => {
      if (shouldUseLisa && typeof geoda === 'function' && dataForLisa.length && mapId.length) {
    ...
    },[typeof geoda, currentData, mapId, shouldUseLisa, JSON.stringify(currentData)])
  ...
  export function useCartogramMap({
    ...
    useMemo(async () => {
      if (shouldUseCartogram && typeof geoda === 'function' && dataForCartogram.length && mapId.length) {
        let cartogramValues = await geoda.cartogram(mapId, dataForCartogram)
          .then(data => data.map((f,i) => ({...f, value: dataForCartogram[i]})))
        setCartogramData(cartogramValues)
      }
    },[JSON.stringify(dataForCartogram), shouldUseCartogram, mapId, typeof geoda])
...
export default function useMapData({
  ...
  const binData = useMemo(() => getDataForBins({
      numeratorData:  dataParams.numerator === 'properties' ? geojsonData?.properties : numeratorData?.data,
      denominatorData: dataParams.denominator === 'properties' ? geojsonData?.properties : denominatorData?.data, 
      dataParams: {...dataParams, nIndex: binIndex, dIndex: dataParams.dType === 'time-series' ? binIndex : null},
      fixedOrder: geojsonData?.order?.indexOrder && Object.values(geojsonData.order.indexOrder),
      dataReady
    })
  ...
  const [
    ...
    dataReady
  }),[JSON.stringify(mapParams), JSON.stringify(dataParams), dataReady, bins, JSON.stringify(lisaData), JSON.stringify(cartogramData)])
